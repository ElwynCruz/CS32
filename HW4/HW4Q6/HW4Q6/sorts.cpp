...
void insertion_sort(vector<Store>& s, bool comp(const Store&, const Store&))
{
	...
	{
		for (int j = i; j > 0 && comp(s[j],s[j-1]) ; j--)									// look through already sorted array and see where this fits
		{																				// 
			swap(s[j], s[j - 1]);														// swap new value backwards into array (s[j] will always have the new value)
...
int main()
{
	...
	// destruction time.
	{
		...
		for (size_t i = 0; i < myStores.size(); i++)
		{
			stores[i] = *(myStores[i]);
		}
